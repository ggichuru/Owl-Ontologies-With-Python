...
onto = get_ontology("testBlightOnto.owl").load()
...
def page_result():
    ...
    with onto_tmp:
        ...
        if temp == "False": blightTemp.isBlightTemp = False

        if humd == "True": blightHumd.isBlightHumd = True
        if humd == "False": blightHumd.isBlightHumd = False

        


        close_world(blightPeriod)
        close_world(blightHumd)
        close_world(blightTemp)
...
werkzeug.serving.run_simple("localhost", 5040, app)